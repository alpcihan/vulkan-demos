...
// end: dithering from https://github.com/hughsk/glsl-dither

void main() 
{
    vec4 col4 = texture(cc1, uv);

    vec3 col = col4.xyz;

    if(ENABLE_DITHERING)
    {
        if(col4.w > 0) col = vec3(0.9,0.5,0.8);
        col = dither8x8(uv * resolution * 0.5 , col);
    } 
