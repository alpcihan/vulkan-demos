...
int main() {
    ...
    std::vector<ModelCPU> modelData = {
        {"dragon.obj", Transform(glm::vec3(-0.5, 0, -0.2), glm::vec3(0, -50, 0), glm::vec3(0.01)), true},
        {"dragon.obj", Transform(glm::vec3(0.5, 0, -0.2), glm::vec3(0, -50, 0), glm::vec3(0.01, 0.01, -0.01)), false}};
    std::vector<SphereLight> sphereLights = {{glm::vec3(1, 0.4, 1.2), glm::vec3(8), 0.2}};
    ...
    tga::Buffer modelCountBuffer;
    {
        ...
        for (int modelIndex = 0; modelIndex < modelCount; modelIndex++) {
            ...
            models.emplace_back(offset, stride, modelDiffuseMapId, model.diffuseColor);
        }
        ...
        // create model count buffer
        {
            tga::StagingBuffer stage =
                tgai.createStagingBuffer({sizeof(uint32_t), reinterpret_cast<uint8_t *>(std::addressof(modelCount))});
    ...
    tga::InputSet imagePassInputSet;
    {
        ...
        /* ray tracing compute pass */
        {
            tga::InputLayout inputLayout({{{
                ...
                tga::BindingType::sampler,                                  // B8: random noise
                tga::BindingType::uniformBuffer,                            // B9: random vec3 buffer
                tga::BindingType::storageBuffer,                            // B10: bvh buffer
                tga::BindingType::storageBuffer                             // B11: triangle idx buffer
            }}});

            std::vector<tga::Binding> bindings = {{modelCountBuffer, 0},
                                                  {modelBuffer, 1},
                                                  ...
                                                  {cameraBuffer, 6},
                                                  {traceStateTexture, 7},
                                                  {randomNoiseTexture, 8},
                                                  {randomUVOffsetBuffer, 9},
                                                  {bvhBuffer, 10},
                                                  {triIdxBuffer, 11}};
